jQuery(document).ready(function(){
    ...
    function addGiftToList($giftData) {
        jQuery.ajax({
            method: 'POST',
            url: 'index.php?option=com_wishee&&task=addGiftAJAX&format=raw',
            data: {
                gift_data: $giftData
            }
        });
    ...
    function populateResults(Product) {
        ...
        SearchResultsDiv.append(
            '<div class="row" style="margin-bottom: 10px; border-bottom: 1px solid #ddd; padding-bottom: 10px;" itemscope itemtype="https://schema.org/Product">' +

                /* Product Image */
                '<div class="col-sm-2">' +
                    '<img style="max-height: 200px; width: auto;" src="' + ImgURL + '" alt="' + Attrbs.Title + '" itemprop="image" />' +
                '</div>' +

                /* Product Title */
                '<div class="col-sm-4">' +
                    '<span itemprop="name">' + Attrbs.Title + '</span>' +
                '</div>' +

                /* Product Category */
                '<div class="col-sm-1">' +
                    '<span itemprop="category">' + ProductGroup + '</span>' +
                '</div>' +

                /* Product Price */
                '<div class="col-sm-1" style="text-align: right;" itemscope itemtype="http://schema.org/Offer">' +
                ...
                '<div class="col-sm-3" style="text-align: right;">' +
                    ...
                    '<a href="#" class="btn btn-primary add-gift-btn" id="' + Product.ASIN + '"><i class="fa fa-gift"></i> Add to List</a>' +
                '</div>' +

            '</div>'
        );
    } /* end: fn.populateResults() */
